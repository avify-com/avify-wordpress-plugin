...
/**
 * Counts orphaned `_sku` postmeta rows, i.e. rows whose post_id does not
 * belong to a product or product_variation. These are usually left behind
 * by WooCommerce/host cleanups and block creating new products that reuse
 * the same SKU.
 *
 * @return int
 */
function avify_count_orphaned_sku_meta() {
    global $wpdb;
...
function avify_render_tools_section() {
    ...
    echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '" onsubmit="return confirm(\'' . esc_js(__('¿Seguro que deseas eliminar los SKUs huérfanos? Esta acción no se puede deshacer.', 'avify-wordpress')) . '\');">';
    wp_nonce_field('avify_clean_orphaned_skus');
    echo '<input type="hidden" name="action" value="avify_clean_orphaned_skus" />';
    submit_button(__('Limpiar SKUs huérfanos', 'avify-wordpress'), 'delete');
    echo '</form>';
}
...
add_action('admin_post_avify_clean_orphaned_skus', 'avify_clean_orphaned_skus_handler');

/** Shows the result of the orphaned SKU cleanup as an admin notice. */
function avify_orphaned_skus_admin_notice() {
    if (!isset($_GET['page']) || $_GET['page'] !== 'avify-settings' || !isset($_GET['avify_orphaned_skus_deleted'])) {
        return;
    }

    $count = (int) $_GET['avify_orphaned_skus_deleted'];
    $message = $count > 0
        ? sprintf(
            /* translators: %d: number of orphaned SKU meta rows deleted. */
            _n('Se eliminó %d SKU huérfano.', 'Se eliminaron %d SKUs huérfanos.', $count, 'avify-wordpress'),
            $count
        )
        : __('No se encontraron SKUs huérfanos.', 'avify-wordpress');

    echo '<div class="notice notice-success is-dismissible"><p>' . esc_html($message) . '</p></div>';
}
add_action('admin_notices', 'avify_orphaned_skus_admin_notice');